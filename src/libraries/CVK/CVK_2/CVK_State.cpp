...
{
    if(indexToRemove >= 0 && indexToRemove < (int)m_lights.size())
...
void CVK::State::setWindow(GLFWwindow *window)
{
    m_window = window;
}

GLFWwindow *CVK::State::getWindow()
{
    return m_window;
}

void
CVK::State::updateSceneSettings(glm::vec3 lightAmbient, int fogMode, glm::vec3 fogCol, float fogStart, float fogEnd,
                                float fogDens)
{
...
void CVK::State::setLightAmbient(glm::vec3 lightAmbient) {
	m_lightAmbient = lightAmbient;
}
