...
KIRK::CPU::SimpleCPURaytracer::SimpleCPURaytracer() : KIRK::GuiElement()
{
	...
	m_num_threads = 1;//KIRK::ThreadManager::maxThreads(); //Maximum thread count
};
...
KIRK::Color::RGBA KIRK::CPU::SimpleCPURaytracer::shadeIntersectionTest(Intersection &hit)
{
	...
	//push input ray to vector
	m_in_rays.push_back(hit.m_ray);

	//R-PATH Test
	/*{
		//create output vector
		glm::vec3 out_ray = glm::reflect(hit.m_ray.m_direction, glm::faceforward(hit.m_normal, hit.m_ray.m_direction, hit.m_normal));//reflect input ray on the surface

		//calculation of tangent vector
		glm::vec3 c1 = glm::cross(hit.m_normal, glm::vec3(0.0, 0.0, 1.0));
		glm::vec3 c2 = glm::cross(hit.m_normal, glm::vec3(0.0, 1.0, 0.0));
		glm::vec3 tangent = (glm::length(c1) > glm::length(c2)) ? glm::normalize(c1) : glm::normalize(c2);
		//Rotate towards normal to account for tilted fiber surface
		out_ray = glm::vec3(glm::vec4(out_ray, 0.f) * glm::rotate(glm::radians(-7.f), tangent));

		//push reflected ray to output rays
		m_out_rays.push_back(KIRK::Ray(hit.m_location + 1e-4f * out_ray, out_ray));
		//push normal to vector 
		m_normal_rays.push_back(KIRK::Ray(hit.m_location, hit.m_normal));
	}*/

	//TT-PATH Test
	/*{
		glm::vec3 t_dir = glm::refract(hit.m_ray.m_direction, glm::faceforward(hit.m_normal, hit.m_ray.m_direction, hit.m_normal), 1.0f / 1.55f);
		glm::vec3 t_normal;
		Ray t_ray(hit.m_location + 1e-4f * t_dir, t_dir);
		Intersection t_hit(t_ray);
		//get the intersection point on the second wall
		if (m_cpuscene->getDataStructure().closestIntersection(&t_hit))
		{
			obj->calcNormal(&t_hit);// calculation of the normal
		...
		//refract on the second wall of the cylinder to get output ray
		glm::vec3 out_ray = glm::refract(glm::normalize(t_dir), glm::faceforward(t_normal, glm::normalize(t_dir), t_normal), 1.0f / 1.55f);
		...
		out_ray = glm::vec3(glm::vec4(out_ray, 0.f) * glm::rotate(glm::radians(-7.f), t_tangent));

		//push first and second refraction ray
		m_out_rays.push_back(t_ray);
		m_out_rays.push_back(KIRK::Ray(t_hit.m_location + 1e-4f * out_ray, out_ray));

		//push first and second intersection normal to vector 
		m_normal_rays.push_back(KIRK::Ray(hit.m_location, hit.m_normal));
		m_normal_rays.push_back(KIRK::Ray(t_hit.m_location, t_normal));
	}*/
	...
	//TRT-PATH Test
	{
		...
		glm::vec3 tr_normal;
		Ray tr_ray(t_hit.m_location + 1e-4f * tr_dir, tr_dir);
		Intersection tr_hit(tr_ray);
		//get the intersection for the second point on the first wall
		if (m_cpuscene->getDataStructure().closestIntersection(&tr_hit))
		{
			obj->calcNormal(&tr_hit);// calculation of the normal
		...
		//TRT-PART
		//refract on the first wall of the cylinder again to get output ray
		glm::vec3 out_ray = glm::refract(glm::normalize(tr_dir), glm::faceforward(tr_normal, glm::normalize(tr_dir), tr_normal), 1.0f / 1.55f);

		//rotate towards normal to account for the tilted fiber surface
		glm::vec3 c1 = glm::cross(tr_normal, glm::vec3(0.0, 0.0, 1.0));
		glm::vec3 c2 = glm::cross(tr_normal, glm::vec3(0.0, 1.0, 0.0));
		glm::vec3 trt_tangent = (glm::length(c1) > glm::length(c2)) ? glm::normalize(c1) : glm::normalize(c2);
		out_ray = glm::vec3(glm::vec4(out_ray, 0.f) * glm::rotate(glm::radians(-7.f), trt_tangent));

		//push all 3 output path rays
		m_out_rays.push_back(t_ray);
		m_out_rays.push_back(tr_ray);
		m_out_rays.push_back(KIRK::Ray(tr_hit.m_location + 1e-4f * out_ray, out_ray));

		//push first and second intersection normal to vector 
		m_normal_rays.push_back(KIRK::Ray(hit.m_location, hit.m_normal));
		m_normal_rays.push_back(KIRK::Ray(t_hit.m_location, t_normal));
		m_normal_rays.push_back(KIRK::Ray(tr_hit.m_location, tr_normal));
	}

	return KIRK::Color::RGBA(0.f, 1.f, 0.f, 1.f);
}
