...
int main(int argc, char *argv[])
{
	...
	scene->addFurFibersToAllMeshes(4, 10, 0.005f);//Hair plane
	...
	cpu_pathtracer->setSampleCount(250);
	...
	while (!window->shouldClose())
	{
		...
		if (m_is_raytracer_active)
		{
			...
			if (m_platform_used == 1)
			{
				//reset clock for this sample
				m_clock.restart();

				//Render with the pathtracer to our texture
				m_active_cpu_raytracer->renderToTexture(render_texture_cpu);

				//add current sample time to totalrendertime
				m_rendertime += m_clock.getElapsedTime<KIRK::Milliseconds>();
		...
		if (m_show_rendertime && cpu_pathtracer->getCurrentSampleCount() == cpu_pathtracer->getMaximalSampleCount())
		{
